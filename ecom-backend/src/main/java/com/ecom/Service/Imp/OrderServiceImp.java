...
public class OrderServiceImp implements OrderService {
	...
	public List<OrderDto> getAllOrder(String p) {
		...
		User findByEmail = this.userRepository.findByEmail(p)
				.orElseThrow(() -> new ResourceNotFoundException("User Not Found"));
		List<Order> findAll = this.orderRepository.findByUser(findByEmail)
				.orElseThrow(() -> new ResourceNotFoundException("Order Not Found"));

		List<OrderDto> collect = findAll.stream().map((each) -> this.mapper.map(each, OrderDto.class))
	...
	public void deleteOrder(int orderId) {
		Order findById = this.orderRepository.findById(orderId)
				.orElseThrow(() -> new ResourceNotFoundException("Order not Found"));
	...
	public List<OrderDto> listAllOrder() {
		...
		List<OrderDto> collect2 = listallorder.stream().map((each) -> this.mapper.map(each, OrderDto.class))
				.collect(Collectors.toList());
